	...
	export default {
		...
		methods:{
			init(orderId){
				...
				if(this.userInfo.userRestaurantLever == 1){
					this.updateIsRead(orderId);
				}
			...
			updateIsRead(orderId){
				this.$http.httpPostByToken('/order/bossReadOrder/' + orderId).then(res=>{
					console.log(res);
				})
			},
			sendComment(){
...
<style lang="scss" scoped src="./me_myOrderDetails.scss"></style>